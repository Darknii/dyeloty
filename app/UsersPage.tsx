...
type ProfileResult = { username: string; avatar_url: string | null };
...
export default function UsersPage({ language }: { language: "en" | "pl" }) {
  ...
  const t = language === "pl" ? { title: "Znajdź użytkownika", intro: "Szukaj po nicku.", placeholder: "np. kasia_dzierga", login: "Przenosimy do logowania…", empty: "Wpisz co najmniej 2 znaki, aby wyszukać użytkownika.", noResults: "Nie znaleziono użytkowników.", error: "Nie udało się wyszukać użytkowników.", profile: "Zobacz profil" } : { title: "Find a user", intro: "Search by username.", placeholder: "e.g. kasia_knits", login: "Taking you to sign in…", empty: "Type at least 2 characters to search users.", noResults: "No users found.", error: "Could not search users.", profile: "View profile" };
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(async () => {
      ...
      const { data, error: searchError } = await supabase.from("profiles").select("username, avatar_url").ilike("username", `%${term}%`).order("username").limit(20).returns<ProfileResult[]>();
      if (!mounted) return;
      setSearching(false);
      if (searchError) { setError(t.error); return; }
      setResults(data ?? []);
    }, 200);
